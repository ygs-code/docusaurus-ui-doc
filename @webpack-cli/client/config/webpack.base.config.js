...
console.log('HTML_WEBPACK_PLUGIN_OPTIONS===', HTML_WEBPACK_PLUGIN_OPTIONS);
...
module.exports = {
  ...
  output: {
    ...
    [hash] or [fullhash] or  [chunkhash] or [contenthash],

    */
    filename: 'static/js/[name]_[contenthash]_bundle.js',
    chunkFilename: 'static/js/[name]_[contenthash]_chunk.js',
    // 访问静态资源目录 比如 css img
  ...
  module: {
    rules: [
      ...
      // {
      //   test: /\.json$/,
      //   type: 'javascript/auto', // 或者使用 'json' 如果你使用的是 json-loader
      //   use: 'json-loader',
      //   // enforce: 'pre',
      // },
      ...
      //处理图片
      //！默认处理不了html中的图片 <img src="./images/BM.jpg" alt=""> 打包后路径不会改变！
      // {
      //   test: /\.(jpg|jpeg|png|gif|webp|svg|bmp|tiff|raw|tag|pcx|pcd)$/,
      //   //只用一个loader  但要下载url-loader 和 file-loader
      //   // loader: 'file-loader', // 处理样式中的url
      //   // // enforce: 'pre',
      //   // options: {
      //   //   //当图片小于8k 会被base64处理
      ...
      //   type: 'asset/resource',
      //   generator: {
      //     filename: 'static/assets/images/[name]_[hash][ext]' // 图片输出路径
      //   }
      // },

      // {
      //   test: /\.(woff|woff2|ttf|ttf2|otf|eot)$/i, // 匹配.woff和.woff2文件
      //    // 不对文件额外处理，直接输出
      //   type: 'asset/resource',
      //   generator: {
      //     filename: 'static/fonts/[name]_[hash][ext]' // 图片输出路径
      //   }
      // },
      ...
      //   test: /\.(woff|woff2|ttf|ttf2|otf|eot)$/i, // 匹配.woff和.woff2文件
      //    // 不对文件额外处理，直接输出
      //   // type: 'asset/resource', // 不对文件额外处理，直接输出
      //   enforce: 'pre',
      ...
      //     loader: 'file-loader',
      //     options: {
      //        name: 'static/fonts/[name].[contenthash:10].[ext]', // 输出文件名格式
      //       // outputPath: 'static/fonts/', // 输出目录
      //       limit: 1024, // 文件大小小于1024byte时作为base64处理
      //       esModule: false,
      //       // publicPath: false, // 禁用动态 publicPath
      //       // generator: {
      //       //   filename: 'static/fonts/[name].[contenthash:10][ext]', // 直接输出文件路径
      //       // },
      //     },
      //   },
      // },
  ...
  plugins: [
    ...
    //         return absoluteFilename.replace(/\\/gi, '/').replace(reg, '/dist/');
    //       },
    //     },
    //     {
    //       from: path.join(process.cwd(), '/public/**/*').replace(/\\/gi, '/'),
    //       to({ context, absoluteFilename }) {
    //         let reg = /\/public\//;
    //         return absoluteFilename.replace(/\\/gi, '/').replace(reg, '/dist/');
    //       },
    //       filter: async (resourcePath) => {
    //         if (resourcePath.search('public/index.html') !== -1) {
    //           return false;
    //         }
    //         return true;
    //       },
    ...
    //     HTML_WEBPACK_PLUGIN_OPTIONS = HTML_WEBPACK_PLUGIN_OPTIONS.replace(
    //       /\s/gi,
    //       '',
    //     );
    //     HTML_WEBPACK_PLUGIN_OPTIONS = HTML_WEBPACK_PLUGIN_OPTIONS.match(regex);
    //     if (HTML_WEBPACK_PLUGIN_OPTIONS) {
    //       HTML_WEBPACK_PLUGIN_OPTIONS = HTML_WEBPACK_PLUGIN_OPTIONS[0];
    //       let htmlWebpackPluginOptionsArr =
    //         HTML_WEBPACK_PLUGIN_OPTIONS.split(',');
    //       HTML_WEBPACK_PLUGIN_OPTIONS = {};
    //       for (let item of htmlWebpackPluginOptionsArr) {
    //         let [key, value] = item.split(':');
    //         if (key) {
    //           HTML_WEBPACK_PLUGIN_OPTIONS[`${key}`] = value;
    //         }
    //       }
    //     } else {
    //       HTML_WEBPACK_PLUGIN_OPTIONS = {};
    //     }
    //     return HTML_WEBPACK_PLUGIN_OPTIONS;
    //   })(),
